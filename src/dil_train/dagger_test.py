...
class ENV:
    def __init__(self,run_name,dev,model,n,direction,controller, human_mode, safety):
    ...
    def choose_model(self):
        if self.controller=='DA-DNN/':
            ...
            print("O-DA-DNN/")
            self.AB_model = '/home/robot/workspaces/Big_Data/nn_train/log/Dagger_DNN/20230303_175706/model.pth'
            self.BA_model = '/home/robot/workspaces/Big_Data/nn_train/log/Dagger_DNN/20230303_221209/model.pth'
    ...
    def done(self):
        ...
        if max_diff<self.threshold_2 or self.counter>500:
            print("-----Arrived------")
            arrive = True
            if self.start[0]==self.A[0]:
                self.model.load_state_dict(torch.load(self.AB_model))
...
if __name__ == '__main__':
    ...
    env = ENV(run_name,dev,model,n,direction,controller,human_mode, safety)
    t = time.time()
    i = 0
    rate = rospy.Rate(20)
    env.choose_model()
