...
class ENV:
    ...
    def train(self):
        ...
        temp = len(x_train)
        for b in range(0, temp, self.n_batch):
    ...
    def evals(self,data, moded):
        total_loss = 0
        model.eval()
        random.shuffle(self.eval_D)
        x_eval = np.array(self.eval_D)[:,0:48]
        y_eval = np.array(self.eval_D)[:,48:54]
        with torch.no_grad():
            for b in range(0, len(x_eval), self.n_batch):
