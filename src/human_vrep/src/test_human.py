...
for k in range (len(pos)):
    temp = pos[k]
    for i in range(rep):
        splited_data.append([temp[start+i*a:i*a+start+10000],k,i*a])
...
class ENV:
    ...
    def step(self,i,data_part,file_number,file_n,file_start,mode):
        k=file_number
        point_array = [0]*465
        for a in range(14):
            if mode=='sit':
                point_array[3*a] = (data_part[i][3*a])+1.5
                point_array[3*a+1] = (data_part[i][3*a+1])+1.5
        ...
        for f in range(10):
            for a in range(14): 
                if mode=='sit':   
                    point_array[f*42+43+3*a] = (data_part[i][3*a])+1.5
                    point_array[f*42+43+3*a+1] = (data_part[i][3*a+1])+1.5
                    point_array[f*42+43+3*a+2] = (data_part[i][3*a+2])+1.2
                else:
                    point_array[f*42+43+3*a] = (data_part[i][3*a])+1.0
                    point_array[f*42+43+3*a+1] = (data_part[i][3*a+1])+d_y
                    point_array[f*42+43+3*a+2] = (data_part[i][3*a+2])
    ...
    if(msg):
        while not rospy.is_shutdown():
            ...
            if condition_h[1]==1:
                ...
                env.step(i,temp,condition_h[0],file_n,file_start,mode)
                print(condition_h,i)
                i+=1
            rate.sleep()
