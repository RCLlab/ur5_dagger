...
#Sitting human test
pos_196 = '/home/robot/Documents/Spheres_Bones_rotated_0.csv'
...
class ENV:
    def __init__(self):
        ...
        self.iter = 0
        self.condition_h = [0]*3
        self.point_array = [0]*464

    def callback(self, data):
        self.condition_h = data.data[0:3]

    def check_condition(self):
        return [int(self.condition_h[0]),self.condition_h[1],self.condition_h[2]]

    def step(self,i,data_part,k,d,f,z=0):
        for a in range(14):
            self.point_array[3*a] = (data_part[i][3*a])+d
            self.point_array[3*a+1] = (data_part[i][3*a+1])+d
            self.point_array[3*a+2] = (data_part[i][3*a+2])+z
        self.point_array[42] = k+1
        for f in range(10):
            for a in range(14):    
                self.point_array[f*42+43+3*a] = (data_part[i][3*a])+d
                self.point_array[f*42+43+3*a+1] = (data_part[i][3*a+1])+d
                self.point_array[f*42+43+3*a+2] = (data_part[i][3*a+2])+z
        self.point_array[463] = f
        obstacle_data = Float64MultiArray()
        obstacle_data.data = self.point_array
    ...
    if(msg):
        while not rospy.is_shutdown():
            ...
            if condition_h[1]==1:
                ...
                if condition_h[2]==0:
                    train_i+=1
                    if train_i>len(pos[train_k])-delay_t:
                        train_k+=1
                        train_i=delay_t
                        if train_k>len(pos)-1:
                            train_i=delay_t
                            train_k=0
                            tr_d+=0.5
                    ...
                    temp = pos[train_k]
                    file_n = train_k
                    print('Train',len(pos[train_k]),train_k,train_i,tr_d)
                    env.step(i=train_i,data_part=temp,k=condition_h[0],d=tr_d,f=file_n)
                
                if condition_h[2]==1:
                    eval_i+=1
                    if eval_i>len(eval_pos[eval_k])-delay_v:
                        eval_k+=1
                        eval_i=delay_v
                        if eval_k>len(eval_pos)-1:
                            eval_i=delay_v
                            eval_k=0
                            ev_d+=0.1
                    temp = eval_pos[eval_k]
                    file_n = eval_k
                    print('Valid',len(eval_pos[eval_k]),eval_k,eval_i,ev_d)
                    env.step(i=eval_i,data_part=temp,k=condition_h[0],d=ev_d,f=file_n)
                
                if condition_h[2]==2:
                    tests_i+=1
                    if tests_i>len(test_sit[tests_k])-delay_ts:
                        tests_k+=1
                        tests_i=delay_ts
                        if tests_k>len(test_sit)-1:
                            tests_i=delay_ts
                            tests_k=0
                            t_d+=0.01
                            
                    temp = test_sit[tests_k]
                    file_n = tests_k
                    print('Test sit',len(test_sit[tests_k]),tests_k,tests_i,t_d)
                    env.step(i=tests_i,data_part=temp,k=condition_h[0],d=t_d,f=file_n,z=1.2)
            rate.sleep()
